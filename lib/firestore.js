...
class Firestore {
	...
	create(model, data, callback) {
		if(!data.id){
			...
			const ref = this.db.collection(model).doc(data.id)
			delete data.id
			ref.set(data).then(ref => callback(null, ref.id)).catch(err => callback(err));
		}
	}
	...
	destroyAll(model, where, callback) {
		...
		/*
		 * Coud delete documents with special value prop
		 * Is used for deleteAll with special value
		 * eg. deleteAll({owner : id})
		*/
		console.log(Object.keys(where).length, '---', where[properties[0]])
		else if(!where.id && Object.keys(where).length == 1) {
			console.log('Delete user projects starts')
			let properties = Object.keys(where);
			let value = where[properties[0]]
			self.db.collection(model).where(properties[0],'==', value).get()
				.then((snapshot) => {
					console.log('snapshot which will be deleted: ', snapshot)
					snapshot.forEach(snap => snap.ref.delete())
					console.log('Delete user projects end')
					callback(null, '');
				})
		}
		callback('_Documents not found');
		// WARNING is commited away from sin
		// else {
